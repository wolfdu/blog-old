...
function getAllDraft({ commit }, tagId) {
  return postsApi.getDraftList(tagId).then(
    res => {
      if (res.success) {
...
function submitDraftTitle({ commit, state }, title) {
  postsApi.modifyDraftTitle(state.currentDraftId, title).then(
    res => {
      if (res.success) {
...
function updateDraft({ commit, state }, content) {
  postsApi.modifyDraftContent(state.currentDraftId, content).then(
    res => {
      if (res.success) {
