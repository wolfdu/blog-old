...
const actions = {
  getPost({ commit }, queryParam) {
    postsApi.getPost(queryParam.postId).then(
      res => {
        if (res.success) {
  ...
  getAllPost({ commit }, param) {
    postsApi.getPostList(param).then(
      res => {
        if (res.success) {
  ...
  submitPostTitle({ commit, state }, title) {
    postsApi.updatePostTitle(state.postId, title).then(
      res => {
        if (res.success) {
          const data = res.data
      ...
      res => {
        showMsg(store, {
          content: res.error_message || '网络错误,标题保存失败'
        })
      }
    )
  },
  ...
  deletePostTag({ commit, state }, deleteTagId) {
    ...
    tagIds.splice(
      tagIds.findIndex(function(id) {
        return id === deleteTagId
      }),
      1
    )
    postsApi.modifyPostTags(state.postId, tagIds).then(
      res => {
        if (res.success) {
          commit(MODIFY_POST_TAG, res.data)
          commit(REFRESH_POSTS, {
      ...
      res => {
        showMsg(store, {
          content: res.error_message || '网络错误,修改标签失败'
        })
      }
    )
  },
  ...
  submitPostContent({ commit }, postContent) {
    postsApi.updatePostContent(state.postId, postContent).then(
      res => {
        if (res.success) {
          const data = res.data
  ...
  publish({ commit, state }) {
    if (state.postSaved && state.postTitleSaved && !state.post.draftPublished) {
      postsApi.publishPost(state.postId).then(
        res => {
          if (res.success) {
      ...
      showMsg(store, {
        content: '当前文章正在保存中或该文章已发布过了，请稍后再试',
        type: 'info'
      })
    }
  },
